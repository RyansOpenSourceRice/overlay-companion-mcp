...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(
            Path.home() / '.local/share/overlay-companion/clipboard-bridge.log'
        )
    ]
)
...
class ClipboardManager:
    ...
    async def set_clipboard(self, content: str, content_type: str = "text/plain") -> bool:
...
async def get_clipboard():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get clipboard content: {str(e)}"
        )
...
async def set_clipboard(clipboard_data: ClipboardContent):
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=500,
                detail="Failed to set clipboard content"
            )
    except Exception as e:
        logger.error(f"Failed to set clipboard: {e}")
        raise HTTPException(
            status_code=500,
            detail=f"Failed to set clipboard content: {str(e)}"
        )
...
async def clear_clipboard():
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=500,
                detail="Failed to clear clipboard"
            )
    except Exception as e:
        logger.error(f"Failed to clear clipboard: {e}")
        raise HTTPException(
            status_code=500,
            detail=f"Failed to clear clipboard: {str(e)}"
        )
