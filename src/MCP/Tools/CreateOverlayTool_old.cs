...
[McpServerToolType]
public class CreateOverlayTool : IMcpTool
{
    private readonly IOverlayService _overlayService;
    private readonly IKasmVNCService _kasmvncService;
    private readonly ILogger<CreateOverlayTool> _logger;

    public string Name => "create_overlay";
    public string Description => "Create a visual overlay on the screen with KasmVNC integration for multi-monitor support";
    ...
    {
        // Convert parameters to strongly typed request
        var request = new CreateOverlayRequest();
        
        if (parameters.TryGetValue("x", out var x))
            request.X = Convert.ToInt32(x);
        if (parameters.TryGetValue("y", out var y))
            request.Y = Convert.ToInt32(y);
        ...
        if (parameters.TryGetValue("opacity", out var opacity))
            request.Opacity = Convert.ToDouble(opacity);
        if (parameters.TryGetValue("label", out var label))
            request.Label = label?.ToString();
        if (parameters.TryGetValue("clickThrough", out var clickThrough))
            request.ClickThrough = Convert.ToBoolean(clickThrough);
        if (parameters.TryGetValue("monitorIndex", out var monitorIndex))
            request.MonitorIndex = Convert.ToInt32(monitorIndex);
        if (parameters.TryGetValue("temporaryMs", out var temporaryMs))
            request.TemporaryMs = Convert.ToInt32(temporaryMs);
