...
interface McpConfig {
  ...
  desktop?: { target: string; viewport: { w: number; h: number; devicePixelRatio: number } };
...
export default class MCPConfigManager {
  ...
  async loadConfiguration(): Promise<void> {
    try {
      ...
      this.config = (await response.json()) as McpConfig;
  ...
  generateFallbackConfig(): McpConfig {
  ...
  showManualCopyInstructions(configText: string): void {
    ...
    const textarea = instructionsModal.querySelector('textarea') as HTMLTextAreaElement | null;
    if (textarea) {
      setTimeout(() => {
        textarea.focus();
        textarea.select();
  ...
  generateGenericMCPConfig(): McpConfig | null {
    if (!this.config) return null;
    return {
      ...this.config,
  ...
  validateConfiguration(): boolean {
    ...
    const required: (keyof McpConfig)[] = ['mcp_version', 'session_id', 'mcp_ws_url', 'auth', 'desktop'];
    ...
    try {
      new URL(this.config.mcp_ws_url as string);
    } catch {
  ...
  async refreshConfiguration(): Promise<McpConfig | null> {
