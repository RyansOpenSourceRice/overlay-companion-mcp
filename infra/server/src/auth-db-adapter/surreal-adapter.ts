    ...
    adapter: ({
      ...
      return {
        ...
        async consumeOne({ model, where }) {
          ...
          return row ? recordIdsToStrings(row) : null;
        },
        ...
        async incrementOne({ model, where, increment, set }) {
          ...
          for (const [k, delta] of Object.entries(increment ?? {})) {
            const col = getFieldName({ model, field: k });
            assignments.push(`${col} = ${col} + ${Number(delta)}`);
          }
          if (set) {
            const serialized = serializeRecordIdFieldsFn(tableName, set);
