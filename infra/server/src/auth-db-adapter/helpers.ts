...
export function buildWhereClauseParts(
  ...
  // Rebuild the special-case mappings and a flat view of the WHERE conditions
  // so query value conversion is consistent with the write path
  // (serializeRecordIdFields). Without this, conditions on special-case
  // reference fields (e.g. account.accountId for credential accounts) are
  // compared as plain strings against record-ids stored by the write path and
  // never match — this broke findCredentialAccount and, in turn, TOTP enable,
  // account deletion password re-auth, and change-password.
  const specialCases = buildSpecialCasesMapping(
    getModelName,
    getFieldName,
    config,
  );
  const whereData: Record<string, any> = {};
  for (const w of where) whereData[w.field] = w.value;

  const resolveReferenceTable = (
    targetTable: string,
    targetField: string,
  ): string | null => {
    const specialCase = specialCases[targetTable]?.[targetField];
    if (
      specialCase &&
      (!specialCase.condition || specialCase.condition(whereData))
    ) {
      return specialCase.recordTable;
    }
    return getReferencedModelFn(targetTable, targetField);
  };
  ...
  where.forEach((w, idx) => {
    ...
    if (operator === "in") {
      ...
      if (internalField === "id") {
        ...
        const referencedModelName = resolveReferenceTable(tableName, fieldName);
