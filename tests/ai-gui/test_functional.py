...
def test_overlay_functionality():
    ...
    try:
        ...
        if not init_result:
            ...
            return False
        print(
            f"✅ Initialize successful: {init_result.get('result', {}).get('serverInfo', {}).get('name')}"
        )
            ...
            if image_data:
                ...
                try:
                    ...
                    if len(decoded) > 1000:  # Reasonable size check
                        ...
                        with open(
                            artifacts_dir / "functional_test_screenshot.png", "wb"
                        ) as f:
                            f.write(decoded)
                        print(
                            "📁 Screenshot saved to artifacts/functional_test_screenshot.png"
                        )
                    else:
        ...
        if overlay_ids:
            remove_result = client.call_tool(
                "remove_overlay", {"overlayId": overlay_ids[0]}
            )
            if remove_result:
        ...
        clipboard_set = client.call_tool(
            "set_clipboard", {"text": "Test clipboard content"}
        )
        if clipboard_set:
