...
APP_BIN = Path(
    os.environ.get("AI_GUI_APP_BIN", ROOT / "build/publish/overlay-companion-mcp")
)
...
def run(
    cmd: str, env: Dict[str, str] | None = None, timeout: int = 60
...
def verify_overlay(img_path: Path, rect: dict) -> dict:
    ...
    try:
        color = avg_color_in_rect(
            str(img_path), (rect["x"], rect["y"], rect["width"], rect["height"])
        )
        res["avg_color"] = color
...
def main():
    ...
    try:
        ...
        summary["ok"] = (
            any(s.get("ok") for s in summary["steps"]) and len(summary["steps"]) > 0
        )
    except Exception as e:
        summary["error"] = str(e)
        summary["ok"] = False
    ...
    try:
        ...
        def pick_tool(names):
            ...
            try:
                tnames = [
                    t.get("name")
                    for t in (tools.get("result", {}).get("tools") or [])
        ...
        draw_name = (
            pick_tool(["draw_overlay", "DrawOverlay", "Overlay.Draw", "overlay/draw"])
            or "draw_overlay"
        )
        take_name = (
            pick_tool(["take_screenshot", "TakeScreenshot", "screenshot/take"])
            or "take_screenshot"
        )
        remove_name = (
            pick_tool(["remove_overlay", "RemoveOverlay", "overlay/remove"])
            or "remove_overlay"
        )
        ...
        if overlay_id:
            rr = client.call_tool(
                remove_name, {"overlayId": overlay_id, "overlay_id": overlay_id}
            )
...
def mcp_roundtrip(app_proc: subprocess.Popen) -> dict:
    ...
    try:
        ...
        try:
            ...
            evidence["set_mode_success"] = not mode_result.get("result", {}).get(
                "isError", False
            )
        except Exception as e:
        ...
        evidence["ok"] = bool(
            init
            and tools
            and tools.get("result", {}).get("tools")
