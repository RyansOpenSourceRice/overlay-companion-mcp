    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if init_result:
                ...
                print(f"⚠️ HTTP Initialize returned 200 but couldn't parse SSE response")
        ...
        if response.status_code == 200:
            ...
            if tools_result:
                ...
                print(f"⚠️ HTTP Tools list returned 200 but couldn't parse SSE response")
        else:
        ...
        if response.status_code == 200:
            ...
            if overlay_result:
                ...
                print(f"⚠️ HTTP Draw overlay returned 200 but couldn't parse SSE response")
        else:
            print(f"❌ HTTP Draw overlay failed: {response.status_code} - {response.text}")
            ...
            if screenshot_result:
                ...
                if 'content' in result_data and result_data['content']:
                    ...
                    if image_data and len(image_data) > 100:
                        print(f"✅ HTTP Screenshot successful: {len(image_data)} chars of base64 data")
                    else:
                        print("⚠️ HTTP Screenshot returned but data seems small")
                else:
                    print("⚠️ HTTP Screenshot returned but no content found")
            else:
                print(f"⚠️ HTTP Screenshot returned 200 but couldn't parse SSE response")
        else:
