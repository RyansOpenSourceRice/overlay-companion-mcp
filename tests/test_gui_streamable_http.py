    ...
    def _send_request(self, method, params=None):
        ...
        try:
            response = self.session.post(
                self.url, json=request, headers=headers, timeout=30
            )
    ...
    def initialize(self):
        ...
        response = self._send_request(
            "initialize",
            {
                ...
                "capabilities": {},
                "clientInfo": {"name": "gui-test-streamable-http", "version": "1.0.0"},
            },
        )
...
def test_keyboard_input(client):
    ...
    result = client.call_tool(
        "type_text", {"text": "Streamable HTTP is the modern MCP transport! 🎉"}
    )
...
def test_remove_overlay(client):
    ...
    result = client.call_tool("remove_overlay", {"overlayId": "test_text_overlay_http"})
...
def main():
    ...
    log("Protocol Version: 2025-03-26 (Streamable HTTP)", Colors.BLUE)
    log(f"Results Directory: {TEST_RESULTS_DIR}", Colors.BLUE)
    log("Display: :99 (Xvfb)", Colors.BLUE)
    ...
    log(
        "   Benefits: Single endpoint, better resumability, stateless support",
        Colors.BLUE,
    )
