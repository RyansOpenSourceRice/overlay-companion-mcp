...
def test_draw_overlay():
    ...
    canvas_result = send_mcp_request("tools/call", {
        "name": "create_overlay",
        "arguments": {
            "id": "draw_canvas",
            "x": 100,
    ...
    draw_commands = [
        ...
        {"type": "line", "x1": 350, "y1": 50, "x2": 450, "y2": 150, "color": "green", "width": 5},
        {"type": "text", "x": 50, "y": 250, "text": "GUI Test Drawing", "color": "black", "size": 24}
    ]
    
    result = send_mcp_request("tools/call", {
        "name": "draw_overlay",
        "arguments": {
            "overlayId": "draw_canvas",
...
def test_clipboard_operations():
    ...
    set_result = send_mcp_request("tools/call", {
        "name": "set_clipboard",
        "arguments": {
            "content": test_text
        }
    ...
    # Get clipboard
    get_result = send_mcp_request("tools/call", {
        "name": "get_clipboard",
        "arguments": {}
    })
...
def test_remove_overlays():
    ...
    if success_count == len(overlays_to_remove):
        ...
        log(f"⚠️  Removed {success_count}/{len(overlays_to_remove)} overlays", Colors.YELLOW)
        return False
